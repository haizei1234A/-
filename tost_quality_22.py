...
import argparse, pandas as pd, numpy as np, math, os, json, sys
...
def summarize_tost(boot: np.ndarray, eps: float) -> Dict[str, float]:
    ...
    # TOST：两单侧
    p_noninf = float(np.mean(boot <= -eps))  # H0: μ_g - μ_b ≤ -ε
    p_notsup = float(np.mean(boot >= +eps))  # H0: μ_g - μ_b ≥ +ε
...
def approx_power_and_n(diff_mean: float, se: float, eps: float, alpha: float = 0.05, power_target: float = 0.8) -> Dict[str, float]:
    ...
    if se <= 1e-12:
        return dict(posthoc_power=1.0, n_for_80pwr=1.0)
    import scipy.stats as st  # 若无 scipy，可改成近似 zcdf
    z_alpha = st.norm.ppf(1.0 - alpha)  # 单侧
    # 设定“备择距离”：Δ_obs + ε（到门槛的距离）
    delta_eff = (diff_mean + eps)
    z = delta_eff / se
    posthoc_power = float(st.norm.cdf(z - z_alpha))
...
def main():
    ...
    # Bootstrap
    boot_by, overall_boot, weights, methods = boot_overall(df, B=args.B, target_mix=target_mix,
                                                           rng=rng, strata_b_override=strata_b_override if args.mbb_b<=0 else {s:args.mbb_b for s in df["stratum"].unique()})
